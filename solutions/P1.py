...
class Model(Module):
  ...
  def load_ckpt(self, fp:Path):
    ...
    if 'check keys':
      missing_keys = state_dict.keys() - ckpt_dict.keys() - {'vqm.states', 'vqm.state'}
  ...
  def save_ckpt(self, fp:Path):
    state_dict: StateDict = self.state_dict()
...
def get_model(
  ...
  ''' model '''
  MODELS = {k: v for k, v in globals().items() if type(v) == type(Model) and issubclass(v, Model) and v not in [Model, AnsatzModel]}
  ...
  if DEBUG:
    print(f'>> [{method}] n_qubits: {model.n_qubits}, n_params: {param_cnt}')
  ...
  for steps in range(1, n_iter+1):
    ...
    if DEBUG and steps % 200 == 0:
      ...
      print(f'[Epoch {steps}/{n_iter}] kl_div: {kl_div(pdist.numpy(), qdist.numpy()).sum():.5f}, l1_loss: {MAE(pdist, qdist):.5g}, ema(loss): {ema_loss:.5g}')
  model.eval()
...
@timer
def benchmark(n_qubits_cases:List[int]):
  METHODS = [k for k, v in globals().items() if type(v) == type(Model) and issubclass(v, Model) and v not in [Model, AnsatzModel]]
  METHODS = [
    #'WState3_wikipedia', 
    #'WState4_toffoli', 
    'WState_qiskit', 
  ...
  for depth in [2, 1, 3]:
    for method in METHODS:
      ...
      for n_qubits in n_qubits_cases:
        ...
        qdist = model.get_prob()
        if qdist.shape[0] != 2**n_qubits: continue    # compat for fixed n_qubits methods
